...
IP Radar 是一款面向算法研发工程师的专利情报 Web 工作台。针对研发工程师在专利检索、技术调研、侵权排查等场景高度依赖人工逐条检索、频繁跨工具切换，导致响应慢、成果难以复用的痛点，以自然语言交互贯通专利检索、竞品监控、技术调研、FTO 智能初筛四类功能场景，旨在全流程提升研发环节的专利情报获取与处理效率，并实现将专利知识产权风险前置识别。

本项目为 FDE 岗位面试场景的 coding 题交付物，相关信息已脱敏处理。在面试场景的时间约束下（开发周期不足 3 天），项目以最小可用 Demo 的形态交付，但保持了完整的前后端实现与全链路可追溯设计，在专利检索、分析、调研等场景已具备初步可用性。

平台技术栈： Next.js + TypeScript + 专利数据开放平台 API + DeepSeek API 

专利数据源：平台对接某专利开放平台 API 获取真实专利数据，内置 Mock 模式支持零配置本地演示。
